CREATE OR ALTER Procedure [dbo].[Employee_GetRow_N]    
	@v_Empcd			varchar(40)
,	@v_Cocd				Char(5)  
,	@v_Div				varchar(40)		='0'
,	@v_Dept				varchar(40)		='0'
...
As  -- Drop Procedure [dbo].[Employee_GetRow_N] '488','01','0','0','0','0','0','0','',''
 /*
...
,	@v_FromDt		Datetime=''
,	@v_ToDt			Datetime=''
As		-- Drop Procedure [dbo].[Employee_LeaveHistory_N]'927','05/01/2023','05/30/2023'
Begin
	...
	,	(Select LvSalary from #tempLvSalary where EmpCd=Emp.Cd)[LvSalYr]
	...
	,	isnull(case (select Convert(varchar(10),Wp_FromDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F')) when '01/01/1900' then '-' else(select CONVERT(varchar(10),Wp_FromDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F'))End,'01/01/1900')[FormatedWPFromDt]
	,	isnull(case (select Convert(varchar(10),Wp_ToDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F')) when '01/01/1900' then '-' else(select CONVERT(varchar(10),Wp_ToDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F'))End,'01/01/1900')[FormatedWPToDt]
	,	isnull(case (select Convert(varchar(10),WOp_FromDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F')) when '01/01/1900' then '-' else(select CONVERT(varchar(10),Wop_FromDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F'))End,'01/01/1900')[FormatedWOPFromDt]
	,	isnull(case (select Convert(varchar(10),WOp_ToDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F')) when '01/01/1900' then '-' else(select CONVERT(varchar(10),Wop_ToDt,101) from empleave where empcd=Emp.Cd and fromdt=@v_FromDt and todt=@v_ToDt and LvStatus in ('N','Y','F'))End,'01/01/1900')[FormatedWOPToDt]
...
,	@v_Typ			Char(1)
 ,	@v_Usercd		char(10)
As	-- Drop Procedure [dbo].[EmpDocuments_GetRow]'',''
Begin
	Select
		ED.EmpCd[EmpCd]
	,	Rtrim(Emp.FName) +' ' +ISNULL(Rtrim(Emp.MName),'') +' ' +ISNULL(Rtrim(Emp.LName),'')[EmpName]
	,	Cod.SDes[DocTypSDes]
	,	ED.DocNo[DocNo]
...
,   @v_Mode			Char(1) =null
,	@v_Active		bit
As		-- Drop Procedure dbo.[Codes_Update]  -- SP_HELP CODES
Begin
...
CREATE OR ALTER procedure [dbo].[UserBranch_GetRow_N] --  Drop Procedure UserBranch_GetRow_N 'Youne' ,'100' ,''
	@v_UserCd	Char(5)
,	@v_CoCd		Char(5)
,	@v_Div		Char(5)
As
...
End
 ...
 Go 
CREATE OR ALTER PROCEDURE [dbo].[GetMenuWithPermissions_N] 
	@UserCd varchar(10)
AS
BEGIN
	SET NOCOUNT ON;
...
,	@v_Div				char(5)
,	@v_Dept				char(10)
As		-- Drop Procedure [dbo].[CompanyProcessApproval_Detail_GetRow]'01','HRPSS2','001','002'
Begin	
	SELECT 
...
End 
 Go 
CREATE OR ALTER
 Procedure [dbo].[CompanyProcessApproval_GetRow_N]
	@v_CoCd				Char(5)
,	@v_ProcessId		Varchar(10)
,	@v_ApplTyp			char(10)
,	@v_Div				char(5)
...
,	@v_Div				Char(5)
,	@v_Dept				Char(10)
As		-- Drop Procedure [dbo].[CompanyProcessApproval_Update]'01','HRPE2','1','10','B','HDTYP0001','Passport first'
Begin	
	Declare @v_Div1				char(5)
	Declare @v_Dept1				char(5)
		if(@v_Div='0' and @v_Dept='0')		
...
,	@v_Mode			Char(1) = null
As
Begin		-- Drop procedure [dbo].[Country_Update]
	set nocount on
...
,	@v_CoCd			Char(5)
,	@v_Mode			Char(1) = null
As		-- Drop procedure Currency_Update
Begin
...
CREATE OR ALTER Procedure [dbo].[Codes_GetRow_N]
	@v_Cd		Char(10)
--,	@v_CoCd		Char(10)
As		-- Drop Procedure dbo.Codes_GetRow 'Bank','02' select * from CodeGroups
Begin
	Select
		CG.sDes[CodeGrpSDes]
...
,	@v_DivCd			Char(5)
 ,	@v_Usercd		char(10)
As		-- Drop Procedure CompanyDocuments_GetRow '100','',''
Begin
	Select
...
As		-- Drop Procedure [dbo].[CompanyLoanTypes_GetRow] 'JL02      ',''
Begin		
	Select											
		Cd
	,	[Des]
...
--,	@v_Usercd	char(10)
As		-- Drop Procedure [dbo].[Branch_GetRow] '','01'
...
CREATE OR ALTER Procedure [dbo].[Users_GetRow_N]
	@v_Cd		Varchar(10)
As		-- Drop Procedure [dbo].[Users_GetRow] ''
...
,	@v_EmpUser		Char(1)
,	@v_Usercd		varchar(10)
As		-- Drop Procedure [dbo].[Empprovisionsadj_GetRow_N]'','6','HR3','U'
Begin	
	Select
		TransNo
	,	TransDt
...
@v_Narr					varchar(200),
@v_EntryBy				char(5)
as  --drop procedure [dbo].[EmpprovisionsadjAppr_Update_N]
begin
...
CREATE OR ALTER procedure [dbo].[Empprovisionsadj_Update_N]
	...
	,	@v_Narr		Varchar(100)
	,	@v_EntryBy	char(5)
as		--DROP procedure [dbo].[Empprovisionsadj_Update_N]'00613','08/21/2016','MHM/384   ','LS','0','111.82','','','MHM/413'
...
End 
 ...
 Go 
CREATE OR ALTER Procedure [dbo].[EmpCalendar_Update_N]
	@v_SrNo			int
,	@v_Empcd		varchar(10)
,	@v_Date			datetime
,	@v_Title		varchar(20)
...
,	@v_Narr		varchar(50)
,	@v_EntryBy		varchar(5)
As			-- Drop Procedure EmpCalendar_Update_N
 Begin
...
Begin	
	IF (SELECT COUNT(*) FROM NotificationMaster WHERE CoCd = @v_CoCd and ProcessId=@v_ProcessId and SrNo=@v_SrNo and DocTyp= @v_DocTyp) = 0
	  Begin
		Insert into NotificationMaster_N
		values(
			@v_CoCd
		,	@v_ProcessId
		,	@v_DocTyp
