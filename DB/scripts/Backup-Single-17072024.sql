...
Begin
	...
	,	DocRef
	,	DocDt
	,	case LvStatus when 'N' then 'Applied'
		when 'Y' then 'Approved'
		when 'F' then 'Confirmed'
		when 'C' then 'Cancelled'
		when 'R' then 'Rejected'
		else 'Rejoined' end[LvStatus]
	,	LvInter
	,	case WP_FromDt
	...
	--,	LvTaken[Days]
	--,	DATEDIFF(DD,FromDt,dateadd(day,-1,JoinDt)) [Days]
	--,	(((DATEDIFF(D, isnull(WOP_FromDt,'01/01/1900'), isnull(FormatedWP_ToDt,'01/01/1900'))+1)+
	...
	From
		...
		and (FromDt>=@v_Dt1 and ToDt<=@v_Dt2)
...
BEGIN
	...
	select Emp.Cd[Cd],Salute,Dept,LocCd,POB,Nat,Relg,Marital,Desg,Dob,Doj,Basic,BasicCurr,FareEligible,LvDays,Status,Imagefile Image,ImageSign,UserCd
	 , isnull(Emp.Fname,'')+' '+isnull(Emp.Mname,'')+' '+isnull(Emp.Lname,'') [Name],	[Emp2].LvStatus [LvStatus]
	 into #Employee
	 from Employee Emp
	  Join
...
As  --Drop Procedure [Empleave_View_Getrow_N]  '','04888','' ,'0','0','HR'   
begin 
	...
	,	(select des from Designation where cd=Desg)[Designation]
	,	Dept[DeptCd]
	,	(select top 1 case WOP_FromDt
		when '01/01/1900' then null
		else WOP_FromDt
		end [WOP_FromDt] from EmpLeaveAppr A where A.TransNo=El.TransNo order by ApprLvl desc  )[Aprroval_WOP_FromDt]
	,	(select top 1 case WOP_ToDt
		when '01/01/1900' then null
		else WOP_ToDt
		end [WOP_ToDt] from EmpLeaveAppr A where A.TransNo=El.TransNo order by ApprLvl desc  )[Aprroval_WOP_ToDt]
	,	(select top 1 case WP_FromDt
		when '01/01/1900' then null
		else WP_FromDt
		end [WP_FromDt] from EmpLeaveAppr A where A.TransNo=El.TransNo order by ApprLvl desc  )[Aprroval_WP_FromDt]
	,	(select  top 1 case WP_ToDt
		when '01/01/1900' then null
		else WP_ToDt
		end [WP_ToDt] from EmpLeaveAppr A where A.TransNo=El.TransNo order by ApprLvl desc   )[Aprroval_WP_ToDt]	
		from    
		EmpLeave el    
	,	CompanyLeave c    
	,	Employee e    
...
IF (SELECT COUNT(*) FROM EmpLeaveAppr WHERE TransNo = @v_TransNo and ApprLvl=@v_ApprLvl) = 0
    Begin
        insert into EmpLeaveAppr(TransNo,ApprLvl,LvApprDays,LvApprBy,LvApprDt,Status,WP_FromDt,WP_ToDt,WOP_FromDt,WOP_ToDt,Narr,EntryBy,EntryDt,Reason,LvApprFor) values
			(
			@v_TransNo,
			@v_ApprLvl,
			@v_LvApprDays,
...
End
 ...
 Go 
CREATE OR ALTER Procedure [dbo].[Update_Provisions]
	 @v_CoCd	Char(5)
--With Encryption
As		-- Drop Procedure [dbo].[Update_Provisions]'01'
Begin
	Declare @Prd 			int
	Declare @Year 			int
	Declare @EDay 			int
	Declare @EmpCd 			char(10)
	Declare @Basic 			numeric(15,4)
	Declare @DBasic 		numeric(15,4)
	Declare @TDays 			int
	Declare @Days 			numeric(9,4)
	Declare @ClDays 		numeric(9,4)
	Declare @Amt	 		numeric(15,4)
	Declare @ClAmt	 		numeric(15,4)
	Declare @Fact 			numeric(7,4)
	Declare @v_EndDate 		datetime
	Declare @v_BeginDate 	datetime
	Declare @v_EosDate 		datetime
	Declare @LTAmt 			numeric(15,4)
	Declare @NoTickets 		numeric(6,3)
	Declare @TravSect 		char(10)
	Declare @TravClass 		char(10)
	Declare @LvPrd 			numeric(4,2)   
	Declare @CumDays		numeric(7,2)
	Declare @CumNoPay		numeric(7,2)
	Declare @MnthDays		Varchar(10)
	Declare @MnthDays1		Varchar(10)
	Declare @FixedDays		Numeric(12,10)
	Declare @AmtDecs		int
	Declare @LSAmt 			numeric(15,4) 
	Declare @AnnLv			numeric(5,2)
	Declare @INDPAY_AL		Char(10)
	Declare @LSPAY_AL		Char(10)
	Declare @Allowance		numeric(15,4)	
	Declare @BasicCd		Char(15)
	Declare @AccDays		int
	Declare @GrFact			numeric(15,4)
	Declare @AnnualLv		Char(10)
	Declare @UnPaidLv 		numeric(5,2)
	Declare @LDays 			numeric(5,2)
	Declare @LvDays 		numeric(5,2)
	Declare @Leaving		datetime
	Declare @LastDay 		int
	Declare @CoCalcBasis	varChar(2)
	select @INDPAY_AL=Val From Parameters Where Cd='INDPAY_AL' and CoCd=@v_CoCd
	select @LSPAY_AL=Val From Parameters Where Cd='LSPAY_AL' and CoCd=@v_CoCd
	...
	CREATE table #Temp---------------------------For the provision adj
	(
		Empcd Char(10),
		Typ Char(10),
		Days numeric(9,4),
	...
	set @FixedDays=Cast(@MnthDays as Float)/Cast(@MnthDays1 as Float)
	--select @CumNoPay=dbo.GetFunc_CumLvDays(@EmpCd)
	--------------------Findind Last day of the  month-------------------
	Exec @EDay=Get_EDay @Prd,@Year
	set @v_BeginDate=rtrim(@Prd) + '/1/'+rtrim(@Year)
	set @v_EndDate=rtrim(@Prd) + '/' + rtrim(@EDay)+'/'+rtrim(@Year) 
	
	
	
		Declare @NoOfDaysInAMonth	Numeric(12,10)
		if @CoCalcBasis='Y'
			set @NoOfDaysInAMonth=@FixedDays
		else if @CoCalcBasis='M'
			set @NoOfDaysInAMonth=@EDay
		else 
			set @NoOfDaysInAMonth=@CoCalcBasis	--- This is done for fixed month days 
		...
		if(@v_CoCd='01' and (select CoName from Company where Cd=@v_CoCd)='Al JESSOUR BLDG. MATERIALS TRADING')
		set @W_days=30
		else exec @W_days = Get_Eday @Prd,@year
		
	---------------------------Delete the provisions whose gratuity,leave salary,leave ticket provisions are inactivated-------------------------------------------

	Delete EmpProvisions Where 
	Yr=@Year and Prd=@Prd 
	and (isnull(ActAmt,0)=0 and isnull(ActDays,0)=0)
	...
	Declare Cur Cursor for Select 
		...
		,	Case Emp.LvPrd
			when 0 then 0
			else (select (LvDays/(LvPrd+1))*12 From Employee Where Cd=Emp.Cd and LvPrd <>0)
			end [AnnLv]
		,	round(Basic*CR.Rate,@AmtDecs)[Basic]
		,	isnull(dbo.GetFunc_ESBPay(@v_CoCd,Emp.Cd,@Prd,@Year,@EDay,round(Basic*CR.Rate,@AmtDecs),@AmtDecs,@BasicCd,'HCPAYGT'),0)[Allowance]
		,	case isnull(Leaving,'01/01/1900')
			when '01/01/1900' then @v_EndDate
			else isnull(Leaving,'01/01/1900')
			end
		From 
			Employee Emp
		,	Currency CR
		Where 
			Emp.CoCd=@v_CoCd
		and	CR.Cd=Emp.BasicCurr
	--	and	CR.CoCd=@v_CoCd
		and	Emp.GT='Y'
		and	(isnull(Leaving,'01/01/1900') = '01/01/1900' or isnull(Leaving,'01/01/1900') >=@v_BeginDate)
		and	Emp.Status not in ('HSTATNP','HSTATTP','HSTATSR','HSTATST')
		and	Emp.Cd not in (select EmpCd From EmpFinalsetllement Where Month(TransDt)<>@Prd or Year(TransDt)<>@Year)
	open Cur
	while 1=1
	begin
		Fetch next From Cur into @EmpCd,@AnnLv,@Basic,@Allowance,@v_EosDate
		If @@Fetch_Status=-1 Break
		select @CumNoPay=dbo.GetFunc_CumLvDays(@EmpCd)
		select @CumDays=@Eday--DateDiff(D,Doj,@v_EosDate)+1 From Employee Where Cd=@EmpCd   commented by janshi ,if nov is fully leave gt for nov will be zero and dec gt will be sum of dec and nov gt
		
		set @GrFact=isnull(round(dbo.GetFunc_GratuityFact(@v_CoCd,'HREOS99',@CumDays),4),0)
		print @GrFact 
		print @EmpCd
		set @ClDays=@GrFact
		if @INDPAY_AL='Y'
			set @ClAmt=isnull(round(((@Basic*(select isnull(PayFactor,1) from CompanyEOSPay where CoCd=@v_CoCd and Typ='HCPAYGT   ' and rtrim(PayTyp)+rtrim(PayCd)=@BasicCd)+@Allowance)*12/365)*@GrFact,@AmtDecs),0)----Change noofdays in a month to 12/365 from madina advice
		else
			set @ClAmt=isnull(round(((@Basic*(select isnull(PayFactor,1) from CompanyEOSPay where CoCd=@v_CoCd and Typ='HCPAYGT   ' and rtrim(PayTyp)+rtrim(PayCd)=@BasicCd)+@Allowance)*12/(365-@AnnLv))*@GrFact,@AmtDecs),0)
			
		set @Days= @ClDays---isnull(dbo.GetFunc_ProvClAmt(@EmpCd,@Prd,@Year,'D','GT'),0)    commented by janshi ,if nov is fully leave gt for nov will be zero and dec gt will be sum of dec and nov gt
		set @Amt= @ClAmt---isnull(dbo.GetFunc_ProvClAmt(@EmpCd,@Prd,@Year,'A','GT'),0)    commented by janshi ,if nov is fully leave gt for nov will be zero and dec gt will be sum of dec and nov gt
		print @Days
		insert into #Temp values(@EmpCd,'GT',@Days/@EDay,@Amt/@EDay)--to calculate gratuity provision adj(one day gratuity provision)
		
	
---------Done by janshi to incorporate Div wise with transfer

		CREATE table dbo.#WorkingDaysEmployeeGT
		(	EmpCd		char(10),
			W_days		numeric(5,2),
			P_Hdays		numeric(5,2),
			UP_Hdays	numeric(5,2),
			DivCd		Char(10),
			SrNO		int identity
		)
		
		insert into dbo.#WorkingDaysEmployeeGT
		select EmpCd,Sum(isnull(w_days,0)),Sum(isnull(P_Hdays,0)),Sum(isnull(UP_Hdays,0))
		,Div from empattendance 
		where EmpCd=@EmpCd
		group by EmpCd,Div 
			
		
		declare @CountGT int
		declare @CurrentGT int
		set @CountGT=(select Count(*) from dbo.#WorkingDaysEmployeeGT )
		set @CurrentGT=1
		
		while(@CurrentGT<=@CountGT)
		begin
			Declare @UpdatedDays 		numeric(9,4)
			Declare @UpdatedAmt	 		numeric(15,4)	
			set @UpdatedDays=(@Days*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeGT where SrNO=@CurrentGT)/(select W_days from dbo.#WorkingDaysEmployeeGT where SrNO=@CurrentGT )))
			set @UpdatedAmt=(@Amt*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeGT where SrNO=@CurrentGT)/(select W_days from dbo.#WorkingDaysEmployeeGT where SrNO=@CurrentGT )))
			Declare @DivCdGT		Char(10)
			set @DivCdGT=(select DivCd from dbo.#WorkingDaysEmployeeGT where SrNO=@CurrentGT)
			exec Update_EmpProvisions @Prd,@Year,@EmpCd,'GT',@UpdatedDays,@UpdatedAmt,'P',@DivCdGT
			
			set @CurrentGT=@CurrentGT+1
		end
		Drop TABLE dbo.#WorkingDaysEmployeeGT
		--exec Update_EmpProvisions @Prd,@Year,@EmpCd,'GT',@Days,@Amt,'P',
...
-------------------------------------------------------------
	...
	Declare Cur Cursor for Select 
			Emp.Cd
		,	round(Basic*CR.Rate,@AmtDecs)[Basic]
		,	isnull(dbo.GetFunc_ESBPay(@v_CoCd,Emp.Cd,@Prd,@Year,@EDay,round(Basic*CR.Rate,@AmtDecs),@AmtDecs,@BasicCd,'HCPAYBN'),0)[Allowance]
		,	case Leaving
			when '1/1/1900' then @v_EndDate
			else Leaving
			end
		From 
			Employee Emp
		...
		and	CR.Cd=Emp.BasicCurr
	--	and	CR.CoCd=@v_CoCd
		and	(Leaving = '1/1/1900' or Leaving >=@v_BeginDate)
		and	Emp.Status not in ('HSTATNP','HSTATTP')
		and	Emp.Bonus='Y'
		and	Emp.Cd not in (select EmpCd From EmpFinalsetllement Where Month(TransDt)<>@Prd or Year(TransDt)<>@Year)
	open Cur
	while 1=1
	begin
		Fetch next From Cur into @EmpCd,@Basic,@Allowance,@v_EosDate
		If @@Fetch_Status=-1 Break
		select @CumDays=DateDiff(D,Val,@v_EosDate)+1 From Parameters Where CoCd=@v_CoCd and Cd='BONUS_STDT'
		set @ClAmt=isnull(round(((@Basic+@Allowance)/365)*@CumDays,@AmtDecs),0)
		set @Amt= @ClAmt-isnull(dbo.GetFunc_ProvClAmt(@EmpCd,@Prd,@Year,'A','BN'),0)
		set @Days= @CumDays-isnull(dbo.GetFunc_ProvClAmt(@EmpCd,@Prd,@Year,'D','BN'),0)

		exec Update_EmpProvisions @Prd,@Year,@EmpCd,'BN',@Days,@Amt,'P'
	end
	Close Cur
	Deallocate Cur */
	--------------------------------Updating Leave Travel Fare ------------------------------------------------------------
	Declare Cur1 Cursor for
		Select 
			Cd
		,	TravSect
		,	TravClass
		,	NoTickets+isnull((select sum(FareFreq) From Dependents Where FareEligible='Y' and EmpCd=E.Cd),0)
		,	LvPrd
		,	LvDays 
		From 
			Employee E 
		...
		and 	CoCd=@v_CoCd  
		and	(isnull(Leaving,'01/01/1900') = '01/01/1900' or isnull(Leaving,'01/01/1900') >=@v_BeginDate)
		and 	E.Cd not in (select Cd From Employee Where Status in ('HSTATNP','HSTATTP','HSTATSR','HSTATST'))
		and	E.Cd not in (select EmpCd From EmpFinalsetllement Where Month(TransDt)<>@Prd or Year(TransDt)<>@Year)
	open Cur1
	while 1=1                    
	begin
		Fetch next From Cur1 into @EmpCd,@TravSect,@TravClass,@NoTickets,@LvPrd,@LvDays
		If @@Fetch_Status=-1 Break
		--select @LTAmt=@NoTickets*max(Fare) From Airfare Where SectCd=@TravSect and Class=@TravClass and (FromDt <=@v_BeginDate or FromDt ='1/1/1900')and (ToDt >= @v_EndDate or ToDt ='1/1/1900')  Chamged by janshhi to get fare from country table
		select @LTAmt=@NoTickets*(Select Isnull(Provisions,0) from country where Cd=(Select Nat from employee where Cd=@EmpCd))--max(Fare) From Airfare Where SectCd=@TravSect and Class=@TravClass and (FromDt <=@v_BeginDate or FromDt ='1/1/1900')and (ToDt >= @v_EndDate or ToDt ='1/1/1900')
		set @LDays=round(@LvDays*@NoTickets/12,4)
		set @LTAmt=round(@LTAmt*@NoOfDaysInAMonth/365,@AmtDecs)--changed by janshi to get no of days in a month

		insert into #Temp values(@EmpCd,'LT',@LDays/@EDay,@LTAmt/@EDay)--to calculate leave ticket provision adj(one day leave ticket provision)



	---------Done by janshi to incorporate Div wise with transfer
		CREATE table dbo.#WorkingDaysEmployeeLT
		(	EmpCd		char(10),
			W_days		numeric(5,2),
			P_Hdays		numeric(5,2),
			UP_Hdays	numeric(5,2),
		...
		insert into dbo.#WorkingDaysEmployeeLT
		select EmpCd,Sum(isnull(w_days,0)),Sum(isnull(P_Hdays,0)),Sum(isnull(UP_Hdays,0)),Div from empattendance 
		where EmpCd=@EmpCd
		group by EmpCd,Div
		
		declare @CountLT int
		declare @CurrentLT int
		set @CountLT=(select Count(*) from dbo.#WorkingDaysEmployeeLT )
		set @CurrentLT=1
		
		while(@CurrentLT<=@CountLT)
		begin
			Declare @UpdatedLDays 			numeric(9,4)
			Declare @UpdatedLTAmt	 		numeric(15,4)	
			
			set @UpdatedLDays=(@LDays*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeLT where SrNO=@CurrentLT)/(case when (@CurrentLT=1 and (select datepart(yyyy,doj)*100+datepart(MM,doj) from Employee where Cd=@EmpCd)=@Year*100+@Prd) then @EDay else (select W_days from dbo.#WorkingDaysEmployeeLT where SrNO=@CurrentLT ) end)))
			set @UpdatedLTAmt=(@LTAmt*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeLT where SrNO=@CurrentLT)/(case when (@CurrentLT=1 and (select datepart(yyyy,doj)*100+datepart(MM,doj) from Employee where Cd=@EmpCd)=@Year*100+@Prd) then @EDay else (select W_days from dbo.#WorkingDaysEmployeeLT where SrNO=@CurrentLT ) end)))
			Declare @DivCdLT		Char(10)
			set @DivCdLT=(select DivCd from dbo.#WorkingDaysEmployeeLT where SrNO=@CurrentLT)
			exec Update_EmpProvisions @Prd,@Year,@EmpCd,'LT',@UpdatedLDays,@UpdatedLTAmt,'P',@DivCdLT
			
			set @CurrentLT=@CurrentLT+1
		end
		Drop TABLE dbo.#WorkingDaysEmployeeLT
		
		--exec Update_EmpProvisions @Prd,@Year,@EmpCd,'LT',@LDays,@LTAmt,'P'
...
-------------------------------------------------------------
	...
	Close Cur1
	Deallocate Cur1
	----------------------------Updating Leave Salary -----------------------------------------------------------
	Declare LCur Cursor for 
		Select 
			Emp.Cd
		,	isnull(Leaving,'01/01/1900')[Leaving]
		,	LvPrd
		,	LvDays
		,	Case Emp.LvPrd
			when 0 then 0
		...
		and	(isnull(Leaving,'01/01/1900') = '01/01/1900' or isnull(Leaving,'01/01/1900') >=@v_BeginDate)
		and	Cr.Cd=Emp.BasicCurr
		and	Emp.Cd not in (select Cd From Employee Where Status in ('HSTATNP','HSTATTP','HSTATSR','HSTATST'))
		and	LvDays<>0
		and	Emp.LS='Y'
		and	Emp.Cd not in (select EmpCd From EmpFinalsetllement Where Month(TransDt)<>@Prd or Year(TransDt)<>@Year)
	open LCur
	while 1=1
	begin
		Fetch next From LCur into @EmpCd,@Leaving,@LvPrd,@LvDays,@AnnLv,@Basic,@LSAmt
		if @@Fetch_Status=-1 Break
		set @LSAmt=@LsAmt+@Basic
		if @Leaving='01/01/1900' 
			set @LastDay=@EDay
		else
			set @LastDay=DateDiff(D,@v_BeginDate,@Leaving)+1
		if @LSPAY_AL='Y'
		begin
			set @Days=round((@LastDay*@LvDays/365),4)
			set @LsAmt=round((@LastDay*@LvDays/365)*(@LSAmt*12/365),@AmtDecs)  
		end
		else
		begin
			set @Days=round((@LastDay*@LvDays/365),4)
			set @LsAmt=round(((@LvDays/365)*@LastDay)*(@LsAmt*12/(365-@AnnLv)),@AmtDecs)
		END
		
		insert into #Temp values(@EmpCd,'LS',@Days/@EDay,@LsAmt/@EDay)--to calculate leave salary provision adj(one day leave salary provision)
		
			
	---------Done by janshi to incorporate Div wise with transfer
		CREATE table dbo.#WorkingDaysEmployeeLS
		(	EmpCd		char(10),
			W_days		numeric(5,2),
			P_Hdays		numeric(5,2),
			UP_Hdays		numeric(5,2),
		...
		insert into dbo.#WorkingDaysEmployeeLS
		select EmpCd,Sum(isnull(w_days,0)),Sum(isnull(P_Hdays,0)),Sum(isnull(UP_Hdays,0)),Div from empattendance 
		where EmpCd=@EmpCd
		group by EmpCd,Div
		
		declare @CountLS int
		declare @CurrentLS int
		set @CountLS=(select Count(*) from dbo.#WorkingDaysEmployeeLS )
		set @CurrentLS=1
		
		while(@CurrentLS<=@CountLS)
		begin
		
			Declare @UpdatedLSDays 			numeric(9,4)
			Declare @UpdatedLSAmt	 		numeric(15,4)	
			set @UpdatedLSDays=(@Days*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeLS where SrNO=@CurrentLS)/(case when (@CurrentLT=1 and (select datepart(yyyy,doj)*100+datepart(MM,doj) from Employee where Cd=@EmpCd)=@Year*100+@Prd) then @EDay else (select W_days from dbo.#WorkingDaysEmployeeLS where SrNO=@CurrentLS )end)))
			set @UpdatedLSAmt=(@LsAmt*((select W_days-isnull(P_Hdays,0)-isnull(UP_Hdays,0) from dbo.#WorkingDaysEmployeeLS where SrNO=@CurrentLS)/(case when (@CurrentLT=1 and (select datepart(yyyy,doj)*100+datepart(MM,doj) from Employee where Cd=@EmpCd)=@Year*100+@Prd) then @EDay else (select W_days from dbo.#WorkingDaysEmployeeLS where SrNO=@CurrentLS )end)))
			
			Declare @DivCdLS		Char(10)
			set @DivCdLS=(select DivCd from dbo.#WorkingDaysEmployeeLS where SrNO=@CurrentLS)
			exec Update_EmpProvisions @Prd,@Year,@EmpCd,'LS',@UpdatedLSDays,@UpdatedLSAmt,'P',@DivCdLS
			set @CurrentLS=@CurrentLS+1
		end
		Drop TABLE dbo.#WorkingDaysEmployeeLS
	--exec Update_EmpProvisions @Prd,@Year,@EmpCd,'LS',@Days,@LsAmt,'P'
...
-------------------------------------------------------------
	...
	-------------------------------------------------Provision Adjustment-------------------------------------------
		...
		insert into dbo.#ProvisionGT
		select ELP.TransNo,EL.TransDt,EL.JoinDt,ELP.EmpCd, ELP.FromDt,ELP.ToDt,ELP.ProvTyp
		from EmpLeave EL
		inner join EmpLeaveProvisions ELP on ELP.TransNo=EL.TransNo
		where DATEPART(YYYY,EL.JoinDt)*100+DATEPART(MM,EL.JoinDt)= @Year*100+@Prd
		
		declare @CountProvisionGT int
		declare @CurrentProvisionGT int
		set @CountProvisionGT=(select Count(*) from dbo.#ProvisionGT )
		set @CurrentProvisionGT=1
		while(@CurrentProvisionGT<=@CountProvisionGT)
		begin	
			declare @ProvRefDoc		char(10)
			declare @ProvRefDt		Datetime
			declare @ProvEmpCd		char(10)
			declare @Purpose		Varchar(50)
			declare @ProvTyp		char(10)
			declare @ProvGTDays		Numeric(9,4)
			declare @ProvGTAmt		Numeric(15,4)
			
			set @ProvRefDoc=(select RefDoc from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT)
			set @ProvRefDt=(select RefDt from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT)
			set @ProvEmpCd=(select EmpCd from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT)
			set @ProvTyp=(select ProvTyp from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT)
			set @Purpose='Prov Adj on '+Convert(Varchar(50),(DATEPART(YYYY,(select JoinDt from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT))*100+DATEPART(MM,(select JoinDt from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT))))
					
			set @ProvGTDays=((select Days from #Temp where Empcd=@ProvEmpCd and Typ=@ProvTyp)*(select datediff(dd,FromDt,ToDt)+1 from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT))
			set @ProvGTAmt=((select Amt from #Temp where Empcd=@ProvEmpCd and Typ=@ProvTyp)*(select datediff(dd,FromDt,ToDt)+1 from dbo.#ProvisionGT where SrNO=@CurrentProvisionGT))			
			exec Update_Empprovisionsadj @v_BeginDate,@ProvRefDoc,@ProvRefDt,@ProvEmpCd,@ProvTyp,@ProvGTDays,@ProvGTAmt,@Purpose,@Purpose
				
			set @CurrentProvisionGT=@CurrentProvisionGT+1
		end
		drop table #Temp
	--DROP TABLE dbo.#ProvisionGT
-------------------------------------------------------------------------------------------------------------------------
...
CREATE OR ALTER procedure [dbo].[EmpTrans_Update]
	@v_EmpCd		Char(10)
,	@v_EdCd			Varchar(30)
,	@v_EdTyp		Varchar(30)         
,	@v_DocRef		Char(10)
...
,	@v_TrnInd		Char(5)
,	@v_EmpDiv		Char(5)=''
As		-- Drop procedure [dbo].[EmpTrans_Update]
Begin
	/*
...
As  -- Drop Procedure [dbo].[Employee_GetRow_N] '1120','01','0','0','0','0','0','0','MGR20','',1,100,''
 /*
...
,	@v_EmpCd1		varchar(40)=''
,	@v_Usercd		varchar(20)
As		-- Drop Procedure [dbo].[EmpLeave_Approval_GetRow_N] '','3','01','MGR50','U','0','0','','MGR50'  -- List all New Leave Applications for approval(Here @v_Type=3)
Begin	
...
BEGIN
	...
	select ROW_NUMBER() OVER (ORDER BY [Date] desc) AS SrNo,* from 
		(select 
			 Fname+' '+isnull(Mname,'')+' '+isnull(Lname,'')+'('+trim(Cd)+')'+' '+(select trim(sdes )from branch where cd=Div)  [Name],Dob[Date],Div,'Birthday'[Type]
		from Employee
		where Active='Y'
		union all
		 select 
			 Fname+' '+isnull(Mname,'')+' '+isnull(Lname,'') +'('+trim(Cd)+')'+' '+(select trim(sdes )from branch where cd=Div) [Name],Doj[Date],Div,'Work Anniversary'[Type]
		from Employee where Active='Y') Emp
		where 
...
@v_EmpCd char(10)
AS --Drop Procedure [dbo].[EmplLoanAndLeaveHistory]'473'

	--select 
	--	convert(varchar(20),FromDt,103)[FormatedFromDt]
	--,	convert(varchar(20),ToDt,103)[FormatedToDt]
	--,	DATEDIFF(dd,FromDt,ToDt) as NoOfDays
	--,	(select Des from CompanyLeave where Cd=LvTyp)[Type]
	--,	Reason 
	--from EmpLeave 
	--where	LvStatus in('F','J') 
	--and		EmpCd=@v_EmpCd
	...
	--,	ApprAmt
	--,	NoInst
	--,	(select des from CompanyLoanTypes where Cd=LoanTyp)[Type]
	--,	Narr
	--from EmpLoan 
	--where	LoanStatus in('D','C') 
	--and		EmpCd=@v_EmpCd
	--order by TransDt desc
	...
	,	(select des from CompanyLoanTypes where Cd=LoanTyp)[Type]
	,	case LoanStatus 
			when 'N' then 'Pending approval' 
			when 'A' then 'Approved'
			when 'C' then 'Cancelled'
			when 'D' then 'Amount Disbursed'
			when 'R' then 'Rejected'
			else 'No Updates' end as [Status]
	,	Narr[Reason]
	from EmpLoan 
	where	LoanStatus in('D','C','A','N','R') 
	and		EmpCd=@v_EmpCd
	...
	,	case LvStatus 
			...
			when 'R' then 'Rejected'
			when 'V' then 'Revised'
			else 'No Updates' end as [LvStatus]
	,	Reason 
	from EmpLeave 
	where	LvStatus in('F','C','N','R','V','Y') 
	and		EmpCd=@v_EmpCd
	order by TransDt desc
...
CREATE OR ALTER Procedure [dbo].[PrePayroll_Process] 
	@v_CoCd		Char(5)		-- Drop Procedure PrePayroll_Process
,	@v_Prd		Char(2)		-- Drop Procedure PrePayroll_GetRow
,	@v_Year		Char(4)		-- Drop Procedure PrePayroll1_GetRow
,	@v_div		char(5)=''		-- For doing a single div payroll



As   --drop Procedure [dbo].[PrePayroll_Process] '01','03','2024','18'
/*
Declare @v_CoCd		Char(5)
Declare @v_Prd		Char(2)
...
Begin
	...
	Declare @SalRound		Char(1)
	Declare @LvUsed 		numeric(5,2)
	Declare @CumLvNOPay		numeric(7,2)
	Declare @OTAmount		numeric(9,3)
	--------------------------------------------------
	Declare @MBasic 			Numeric(9,3)
	Declare @EmpTyp				Char(10)
	Declare @SalTypCal			char(1)='N'
	Declare @BasicPayCd			Char(5)--checking if the salary calculation based on employee type
	Declare @BasicCd			Varchar(20)
	Declare @EMPTYP_Branch			Char(10)
	-----------------------------------------------------------------------------------------------
	--If sal calculation is based on employee type then the over time salary should be adjusted to basic salary
	-----------------------------------------------------------------------------------------------
	------------------------------------------------------------------------------------------------------
	...
	Set @SalRound = 'N'
	Select @SalRound = Isnull(Val,'N') from Parameters where Cd='SALROUND' and CoCd=@v_CoCd
	Exec @Eday = Get_Eday @Month,@Year
	select @Month
	select @Year
	select @Eday
    Exec Update_Process_EmpJobCard_Attendance @v_CoCd, '', @v_Prd, @v_Year
 
    Delete from EmpTrans where TrnInd='S' and EmpCd not in (Select EmpCd from EmpAttendance where Prd=@Period and TrnInd='S' )--added to delete all entries from emptrans whose attendance deleted // by janshi
	...
	Declare Cur Cursor for
		...
		Where
			Emp.Cd=EAD.EmpCd
		and	CoCd=@v_CoCd And Prd=@Period
		and TrnInd='S'
		and (Ead.Div=@v_div or @v_div='0')
	Open Cur
	While 1=1
	  Begin
		Fetch next from Cur into @EmpCd,@Branch,@Status
		if @@Fetch_Status=-1 Break
		if @Status='HSTATNP'
			Delete from EmpTrans where EmpCd=@EmpCd and TrnInd='S'
		else
			Exec Update_ProcessAttendance @EmpCd,@Period,@Branch--added div by janshi for month between transfer
	  End
	Close Cur
	Deallocate Cur

	Delete from EmpTrans Where EmpCd in (select Cd from Employee Where DOJ >cast(rtrim(@Month)+'/'+rtrim(@EDay)+'/'+rtrim(@Year) as datetime) and CoCd=@v_CoCd and (Div=@v_div or @v_div='0'))
	Delete from EmpTrans Where EmpCd in (select Cd from Employee where Status='HSTATNP' and CoCd=@v_CoCd and (Div=@v_div or @v_div='0') )
 	Delete from EmpAttendanceDetail Where EmpCd in (select Cd from Employee where Status='HSTATNP' and CoCd=@v_CoCd and (Div=@v_div or @v_div='0'))
	--------------Processing GL & HR Link----------------

	Delete from EmpTransDetail where TrnInd='S'
	Update EmpTrans set CoCd=(select CoCd from Employee where Cd=EmpTrans.EmpCd) where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd and (Div=@v_div or @v_div='0')) 

	
	-------------------------------Commented by janshi,div is already set as in emptrans from empattendance if there is any transfer---------------------------------------
	--Update EmpTrans set HrDiv=--(select Div from Employee where Cd=EmpTrans.EmpCd) where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	--		(select (case when(select COUNT(*) from EmpTransfers where EmpCd=EmpTrans.EmpCd
	--			and DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year)=0
	--		then
	--			(select Div from Employee where Cd=EmpTrans.EmpCd) 
	--		else
	--			(select top 1 BrFrom from EmpTransfers 
	--			where DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year and EmpCd=EmpTrans.EmpCd
	--			order by TransferDt asc)
	--		end))
	--			where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	---------------------------------------------------------------------------------------------------------------------------------			
	--Updating Emptrans based on the Employee Type--------------------------------------------------------------------------------------
	select @SalTypCal=isnull(val,'N') from Parameters where cd='EMPTYP_SAL_ENABLE'
	if(@SalTypCal='Y')
	Begin
	select @OTAmount=amt from emptrans where edcd='OT' and EdTyp='HEDT01'
	Declare Cur Cursor for
		Select
			Distinct(Empcd)
		,	Amt
		...
		Where
			Emp.Cd=ET.EmpCd
		--and	FromDt >= rtrim(@Month) + '/' + rtrim(@EDay)+'/'+rtrim(@Year)
		and Emp.EmpTyp in('J_ETOJS') and ET.EdCd='OT' and ET.EdTyp='HEDT01' and (HRDiv=@v_div or @v_div='0')
		...
		--Employee Emp,
		--EmpAttendance EA
		--where  Emp.EmpTyp in('J_ETOJS') and ea.EmpCd=Emp.cd
		
	Open Cur
	While 1=1
	  Begin
		Fetch next from Cur into @EmpCd,@OTAmount,@EmpTyp,@EMPTYP_Branch
		if @@Fetch_Status=-1 Break
		if @EmpTyp='J_ETOJS'
		Begin
			Select @BasicPayCd=rtrim(Val),@BasicCd=rtrim(Val) From Parameters Where Cd='BASIC' and CoCd='#'
			select @MBasic=amt from emptrans where empcd=@EmpCd and EdCd=@BasicCd and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
			print @MBasic
			if(@OTAmount>@MBasic)
			Begin
				update EmpTrans set Amt=@OTAmount-@MBasic where EmpCd=@EmpCd and EdCd='OT' and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
			End
			else if(@OTAmount<@MBasic)
			Begin
				if(@OTAmount=0)
				begin
				update EmpTrans set Amt=0 where EmpCd=@EmpCd and EdCd=@BasicCd and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
				update Emptrans set Amt=0 where EmpCd=@EmpCd and EdCd='OT' and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
				end
				else
				begin
				update EmpTrans set Amt=@OTAmount where EmpCd=@EmpCd and EdCd=@BasicCd and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
				update Emptrans set Amt=0 where EmpCd=@EmpCd and EdCd='OT' and EdTyp='HEDT01' and HRDiv=@EMPTYP_Branch
				end
			End
		End
	...
	Update EmpTrans set HrCC=(select CC from Employee where Cd=EmpTrans.EmpCd) where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	Update EmpTrans set HrDept=--(select Dept from Employee where Cd=EmpTrans.EmpCd) where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
			(select (case when(select COUNT(*) from EmpTransfers where EmpCd=EmpTrans.EmpCd
				and DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year)=0
			then
				(select Dept from Employee where Cd=EmpTrans.EmpCd) 
			else
				(select top 1 DeptFrom from EmpTransfers 
				where DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year and EmpCd=EmpTrans.EmpCd
				order by TransferDt asc)
			end))
				where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
				
	Update EmpTrans set LocCd=--(select LocCd from Employee where Cd=EmpTrans.EmpCd) where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
			(select (case when(select COUNT(*) from EmpTransfers where EmpCd=EmpTrans.EmpCd
				and DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year)=0
			then
				(select LocCd from Employee where Cd=EmpTrans.EmpCd) 
			else
				(select top 1 LocFrom from EmpTransfers 
				where DATEPART(mm, TransferDt)>@Month and DATEPART(YYYY, TransferDt)>=@Year and EmpCd=EmpTrans.EmpCd
				order by TransferDt asc)
			end))
				where EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	...
	Insert into EmpTransDetail(EmpCd,EdTyp,EdCd,TrnInd,SrNo,LnNo,Curr,ExRate,Amt,HRDiv,HRCC,HRDept)
	(select EmpCd,EdTyp,EdCd,TrnInd,SrNo,1,Curr,ExRate,Amt,HRDiv,HRCC,HRDept from EmpTrans where TrnInd='S')

	Update EmpTransDetail set HRDiv=(select HRDiv from EmpTrans where EmpCd=EmpTransDetail.EmpCd and EdTyp=EmpTransDetail.EdTyp and EdCd=EmpTransDetail.EdCd and HRDiv=EmpTransDetail.HRDiv and SrNo=EmpTransDetail.SrNo) where TrnInd='M' and EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) --added div by janshi for month between transfer
	Update EmpTransDetail set HRCC=(select HRCC from EmpTrans where EmpCd=EmpTransDetail.EmpCd and EdTyp=EmpTransDetail.EdTyp and EdCd=EmpTransDetail.EdCd and HRDiv=EmpTransDetail.HRDiv and SrNo=EmpTransDetail.SrNo) where TrnInd='M' and EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	Update EmpTransDetail set HRDept=(select HRDept from EmpTrans where EmpCd=EmpTransDetail.EmpCd and EdTyp=EmpTransDetail.EdTyp and EdCd=EmpTransDetail.EdCd and HRDiv=EmpTransDetail.HRDiv and SrNo=EmpTransDetail.SrNo) where TrnInd='M' and EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	Update EmpTransDetail set Curr=(select Curr from EmpTrans where EmpCd=EmpTransDetail.EmpCd and EdTyp=EmpTransDetail.EdTyp and EdCd=EmpTransDetail.EdCd and HRDiv=EmpTransDetail.HRDiv and SrNo=EmpTransDetail.SrNo) where TrnInd='M' and EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 
	Update EmpTransDetail set ExRate=(select ExRate from EmpTrans where EmpCd=EmpTransDetail.EmpCd and EdTyp=EmpTransDetail.EdTyp and EdCd=EmpTransDetail.EdCd and HRDiv=EmpTransDetail.HRDiv and SrNo=EmpTransDetail.SrNo) where TrnInd='M' and EmpCd in (Select Cd from Employee where CoCd=@v_CoCd) 



	---------Inserting AcCd for EmpTransDetail---------------------
	Update EmpTransDetail
		Set
			AcCd=null
		,	Div=null
		,	CC=null
		...
		Where
			TrnInd='S'
	Declare @HRDiv Char(5)
	Declare @HRCC Char(10)
	Declare @HRDept Char(10)
	Declare @AcCd Char(10)
	Declare @Div Char(5)
	Declare @CC Char(10)
	Declare @Dept Char(10)
	Declare @EdTyp Char(10)
	Declare @EdCd Char(5)
	...
	While 1=1
	   Begin
		Fetch next from AccCur into @EdTyp,@EdCd,@HRDiv,@HRCC,@HRDept,@AcCd,@Div,@CC,@Dept
		if @@Fetch_Status=-1 Break
		Update EmpTransDetail
			Set 
				AcCd=@AcCd
			...
			and	EdCd=@EdCd
			and	HRDiv=@HRDiv
			and	isnull(HRCC,'N')=isnull(@HRCC,'N')
			and	isnull(HRDept,'N')=isnull(@HRDept,'N')
			and	TrnInd='S'
	   End
	Close AccCur
	Deallocate AccCur
	----------------------------------------------------------------------
--	Exec GetRow_Round @v_CoCd,@EmpCd
	If @SalRound = 'Y'
		Exec Update_Generate_EmpSalRound @v_CoCd,''
	Else
		Delete from EmpSalRound Where EmpCd in(Select Cd from Employee where CoCd=@v_CoCd)
--	Exec GetRow_ProcessPaySlip
	Exec Update_ProcessPaySlip @v_CoCd,@v_div
	Exec Update_ProcessBankTransfer @v_CoCd
	Exec Update_Provisions @v_CoCd
	
	
	----------------------------------------------Leave Master Entry-------------------------------------
	
	--CREATE table #TempLeaveMaster(
	--Empcd		Char(10),
	--LvUsed		numeric(5,2),
	--CumLvNoPay numeric(7,2))
 
	--insert into #TempLeaveMaster 
	--	select  Empcd,
	--		((select case when DATEPART(MM,WP_ToDt)=@Month then DATEPART(dd,WP_ToDt) else @EDay end)-
	--		(select case when DATEPART(MM,WP_FromDt)=@Month then (DATEPART(dd,WP_FromDt)-1) else 
	--				(select case when DATEPART(MM,WP_FromDt)>@Month then @EDay else 0 end) end))[LvUsed],
	--		((select case when DATEPART(MM,WOP_ToDt)=@Month then (DATEPART(dd,WOP_ToDt)-1) else @EDay end)-
	--		(select case when DATEPART(MM,WOP_FromDt)=@Month then DATEPART(dd,WOP_FromDt) else 
	--			(select case when DATEPART(MM,WOP_FromDt)>@Month then @EDay else 0 end) end))[CumLvNoPay]
	--	from EmpLeave 
	--	where (@Year*100)+@Month
	--		between (datepart(YYYY,FromDt)*100)+datepart(MM,FromDt) and (datepart(YYYY,ToDt)*100)+datepart(MM,ToDt)
	--		and lvstatus='J'
			
	--Declare Curs Cursor for
	--	select  Empcd
	--			,sum(LvUsed)[LvUsed]
	--			,sum(CumLvNoPay)[CumLvNoPay]
	--	from #TempLeaveMaster 
	--	group by Empcd
	--Open Curs
	--While 1=1
	--  Begin
	--	Fetch next from Curs into @EmpCd,@LvUsed,@CumLvNOPay
	--	print @@Fetch_Status
	--	if @@Fetch_Status=-1 Break
	--		EXEC EmpLeaveMaster_Update @EmpCd,'AL',0,0,@LvUsed,0,@CumLvNOPay,0,0,'','b'
	--  End
	--Close Curs
	--Deallocate Curs
	--drop table #TempLeaveMaster
	
End

 
 Go 
CREATE OR ALTER     Procedure [dbo].[GetRepo_PaySlip_N] 
	@v_CoCd			char(5)
,	@v_Div			char(5)
,	@v_RPrd			Char(2)
,	@v_RYear		Char(4)
,	@v_EmpCd		Char(10)
As		-- Drop Procedure [dbo].[GetRepo_PaySlip_n]'01','All','03','2024','om 48'
Begin
/*
	Declare @v_CoCd char(5)
...
if cast(@v_RPrd as int) = @Prd and cast(@v_RYear as int)=@Year
	Begin
	  ...
	  --,	@v_CoName[Co.Name] 
		...
		,	(select Isnull(sum(W_Days),0)-(Isnull(sum(P_HDays),0)+Isnull(sum(Up_HDays),0)) from EmpAttendance where Empcd=Tp.EmpCd and prd=@Period)[W_days]
		,	convert(int,(Select Isnull(sum(Up_HDays),0) From EMPATTENDANCE Where EMPCD=TP.EMPCD AND PRD=@Period)) [Up_Hdays]
		, case  trnind	when 'M' then (select narr from EmpTrans where empcd=tp.EmpCd and edcd =tp.EdCd and edtyp=tp.EdTyp  )
			else '' end [Narr1]
		, case  trnind1	when 'M' then (select narr from EmpTrans where empcd=tp.EmpCd and edcd =tp.EdCd1 and edtyp=tp.EdTyp1  )
			else '' end [Narr2]
		From
			TempPaySlip Tp
		,	Employee Emp
...
--			,	Isnull(dbo.GetFunc_OTCaption(Emp.Cd,'HCOTT0003',@ROT3C),'')[ODays]
		...
		,	(select Isnull(sum(W_Days),0)-(Isnull(sum(P_HDays),0)+Isnull(sum(Up_HDays),0)) from EmpAttendanceytd where Empcd=Tp.EmpCd and prd=@Period)[W_days]
		,	convert(int,(Select Isnull(sum(Up_HDays),0) From EmpAttendanceytd Where EMPCD=TP.EMPCD AND PRD=@Period)) [Up_Hdays]
		, case  trnind	when 'M' then (select narr from EmpTransYtd where empcd=tp.EmpCd and edcd =tp.EdCd and edtyp=tp.EdTyp and Prd=@Period )
			else '' end [Narr1]
		, case  trnind1	when 'M' then (select narr from EmpTransYtd where empcd=tp.EmpCd and edcd =tp.EdCd1 and edtyp=tp.EdTyp1 and Prd=@Period )
			else '' end [Narr2]
		From
			PaySlipYtd Tp
		,	Employee Emp
...
--select * From EmpAttendance where up_hdays>0
...
,   @v_Department   Char(10)
,   @v_Sponsor      Char(10)
As				-- Drop Procedure [GetRepo_EmpPayDetail_Format_N]'01','02','2024','All','01','All ','All','All'
...
,		@query  AS NVARCHAR(MAX)
	If Cast(@v_RYear as int) *100 +Cast(@v_RPrd as int) >= @Year *100 +@Month
		...
		select 
		EmpCd,
		case edtyp when 'HEDT01' then sum(amt)
					when 'HEDT02' then -sum(amt)
					when 'HEDT03' then sum(amt) end[Amt],
		EdTyp 
		into #emptrans1 from emptrans  
		group by EdTyp,empcd
		select empcd,sum(amt)[Amt] into #emptrans2 from #emptrans1  group by empcd

			Select 	distinct
				EmT.Empcd[Cd]	
			,	rtrim(isnull(Emp.Fname,'')) +' '+rtrim(isnull(Emp.MName,''))+' '+rtrim(isnull(Lname,''))   [EmpName] 
			,	(select des from Branch where cd=EmT.HRDiv)[Branch]
			,	(select des from codes where typ='ESPON' and cd=emp.Sponsor)[Sponsor]
			,	(select Personal_No from Employee where  cd=EmT.EmpCd)[PersonId]
			,	(select Des from SysCodes where Cd='HPMOD0001 ')[Paymode]
			...
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='052' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[TraAllow]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='FOTA' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[FOTA]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='051' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Living.Allow]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='004' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Other Allowance]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='INCRM' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Incr.Allow]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd in('001','052','FOTA','051','004','INCRM') and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Total Salary]

			,	round((select isnull(sum(amt),0) from emptrans where EdCd='OT' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[OT]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='LSA' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[LSA]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='LTA' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[LTA]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='MGRIN' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Mngr.Inc]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='PRDIN' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Prod.Inc]
			,	round((select isnull(sum(amt),0) from EmpTrans where EdCd='207 ' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  ),2)[Incentives]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='053' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[LastSalary]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='008  ' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[AirFare]
			,	round((select isnull(sum(amt),0) from emptrans where EdCd='204' and EdTyp='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) ),2)[Rent]

			,	round(
			(select isnull(sum(amt),0) from emptrans where EdCd in('001','052','FOTA','051','004','INCRM','OT','LSA','LTA','MGRIN','PRDIN','207','053','008') and EdTyp in('HEDT01','HEDT03') and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv))
				-
			((select isnull(sum(amt),0) from emptrans where EdCd='204' and EdTyp ='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv))
			),2)[TOTAl GROSSSALARY]
			...
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTrans where EdCd='TMFC' and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)),0),2)[TMFCG]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTrans where EdCd in('PENSI')   and EdTyp='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)),0),2)[Pension]
			,	(Select Amt from #emptrans2 where empcd=Emp.cd)[NET SALARY]
			...
			drop table  #emptrans1
			drop table  #emptrans2
		END
				Else
		BEGIN
			Set	@Month=@v_RPrd
		...
		case edtyp when 'HEDT01' then sum(amt)
					when 'HEDT02' then -sum(amt)
					when 'HEDT03' then sum(amt) end[Amt],
		EdTyp,prd into #emptrans3 from EmpTransYtd  
		group by EdTyp,empcd,Prd
		select empcd,sum(amt)[Amt],prd into #emptrans4 from #emptrans3  group by empcd,Prd
			...
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='052' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[TraAllow]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='FOTA' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[FOTA]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='051' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[Living.Allow]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='004' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[Other Allowance]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='INCRM' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[Incr.Allow]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd in('001','052','FOTA','051','004','INCRM') and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[Total Salary]

			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='OT' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[OT]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='LSA' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[LSA]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='LTA' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[LTA]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='MGRIN' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[Mngr.Inc]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='PRDIN' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[Prod.Inc]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='207 ' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),2)[Incentives]

			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='053' and EdTyp='HEDT01' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[LastSalary]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='008  ' and EdTyp='HEDT03' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd ),2)[AirFare]
			,	round((select isnull(sum(amt),0) from EmpTransYtd where EdCd='204' and EdTyp='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),2)[Rent]
			,	round(
			(select isnull(sum(amt),0) from EmpTransYtd where EdCd in('001','052','FOTA','051','004','INCRM','OT','LSA','LTA','MGRIN','PRDIN','207','053','008') and EdTyp in('HEDT01','HEDT03') and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd)
				-
			((select isnull(sum(amt),0) from EmpTransYtd where EdCd='204' and EdTyp ='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd)
			),2)[TOTAl GROSSSALARY]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='208'  and EdTyp='HEDT01' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[StaffAdvGiven]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='211'  and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[StaffAdvCollected]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='FUNDW'  and EdTyp='HEDT01' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[StaffFundGiven]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='STFFP'  and EdTyp='HEDT01' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[StaffFundCollected]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='TMFPA'  and EdTyp='HEDT03' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[TMFPay]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='TMF1' and EdTyp='HEDT02' and EmpCd=EmT.Empcd   and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[TMF1]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='TMF2' and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv) and Prd=@Prd),0),2)[TMF2]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='TMF3' and EdTyp='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),0),2)[TMF3]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd='TMF4' and EdTyp='HEDT02' and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),0),2)[TMF4]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransytd where EdCd='TMFC' and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),0),2)[TMFCG]
			,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd in('PENSI')   and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=EmT.HRDiv)  and Prd=@Prd),0),2)[Pension]
			
			,	(Select Amt from #emptrans4 where empcd=EmT.EmpCd and Prd=@Prd)[NET SALARY]
		...
		Select 	distinct
			...
			drop table  #emptrans3
			drop table  #emptrans4
...
CREATE OR ALTER       PROCEDURE [dbo].[VerifyExistingLvApplication_N]
	...
	@StartDt varchar(20),
	@EndDt varchar(20)
AS-- VerifyExistingLvApplication_N '362','05/29/2024','06/05/2024'
BEGIN
    SET NOCOUNT ON;
...
CREATE OR ALTER     Procedure [dbo].[ActivityLogHead_Getrow]
	...
	@v_EndDate		datetime=''
As		-- Drop Procedure [dbo].[ActivityLogHead_Getrow] '1','',''
Begin
	select * from ActivityLogHead 
	where (ActivityId = @v_ActivityId or @v_ActivityId='')
		and	(((convert(varchar(10),StartTime ,101)>= @v_StartDate or isnull(@v_StartDate,'')='') and( convert(varchar(10),EndTime ,101)<=@v_EndDate or isnull(@v_EndDate,'')=''))) 
End 
 Go 
...
AS
--Drop Procedure [dbo].[EmplLoanAndLeaveApproval_N]'01','488','E'

select count(distinct EmpCd ) as LeaveApplied from EmpLeave
inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
where  LvStatus='N' and emp.CoCd=@v_CoCd 
and emp.div in(select div from UserBranch where usercd=@v_Usercd) and emp.Status in ('HSTATPM','HSTATTG','HSTATTP')
--GETDATE() between FromDt and ToDt and
select count(distinct EmpCd ) as OnLeave from EmpLeave
inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
where ( isnull(JoinDt,'')='') and LvStatus='F' and emp.CoCd=@v_CoCd 
and emp.div in(select div from UserBranch where usercd=@v_Usercd) and emp.Status in ('HSTATPM','HSTATTG','HSTATTP')
--(GETDATE() between FromDt and ToDt) or

--select EmpCd [Working] into #Working from EmpLeave
--inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
--where GETDATE() between FromDt and ToDt and LvStatus='F' and emp.CoCd=@v_CoCd 
--and emp.div in(select div from UserBranch where usercd=@v_Usercd) and emp.Active='Y' and emp.Status in ('HSTATPM','HSTATTG','HSTATTP') --and emp.Status='HSTATPM   '

--select count(cd) [Working] from Employee where cd not in (select [Working] from #Working) and Active='Y' and Status in ('HSTATPM','HSTATTG','HSTATTP')  --where Status='HSTATPM   '
--drop table #Working

select count(cd)[Working] from Employee where  
 cd not in(select distinct EmpCd from empleave where LvStatus='F' and ISNULL(JoinDt,'01/01/1900')='01/01/1900' )
and Status  in ('HSTATPM','HSTATTG   ','HSTATTP   ')
and active='Y'--1225
...
SELECT 
    ...
    ISNULL(SUM(Amt), 0) AS PayElementDetailsCount
FROM 
    (SELECT DISTINCT Prd FROM EmpTransYtd ) p
CROSS JOIN 
    (SELECT DISTINCT EdTyp FROM EmpTransYtd ) e
LEFT JOIN 
    EmpTransYtd t ON p.Prd = t.Prd AND e.EdTyp = t.EdTyp AND t.EdCd <> 'HEDT04' 
GROUP BY 
       p.Prd, e.EdTyp
order by prd

--Employee Dashboard salary details graph
if(@v_Typ='E')--if the empcd passed is employee then only thi salary data display will happen
Begin
SELECT 
    p.Prd,
    ISNULL(SUM(Amt), 0) AS PayElementDetailsCount
...
--inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
--where ((GETDATE() between FromDt and ToDt) or isnull(JoinDt,'')='') and LvStatus='F' and emp.CoCd=@v_CoCd and emp.Status='HSTATPM'

select count(distinct EmpCd ) as OnLeave from EmpLeave
inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
where ((GETDATE() between FromDt and ToDt) or isnull(JoinDt,'')='') and LvStatus='F' and emp.CoCd=@v_CoCd 
and emp.Status='HSTATPM'
...
Begin
	...
	--,	Emp.DOJ	[DOJ]
	,	CONVERT(varchar(20),Emp.DOJ,101)[DOJ]
...
begin
	Begin
	   delete from UserWidgets where UserCd = @v_UserCd and (Widget_Id=@v_Widget_Id or @v_Widget_Id = '')
    End
End 
...
As	-- Drop Procedure [dbo].[Employee_LeaveHistory_GetRow_N] '01','954','05/15/2024'
/*
...
select 
	...
	, isnull(Emp.Fname,'')+' '+isnull(Emp.Mname,'')+' '+isnull(Emp.Lname,'') [EmpName]
...
CREATE OR ALTER PROCEDURE [dbo].[EmpDocIssueRcpt_GetRow]
	@v_EmpCd		char(10)
,	@v_DocTyp		char(10)
,	@v_Typ			char(1)
,	@v_SrNo			numeric(5)
,	@v_LoginEmpCd		Char(10)
,	@v_EmpUser		Char(1)
--drop PROCEDURE [dbo].[EmpDocIssueRcpt_GetRow]'','','1','0','001','U'
as
select 
			
		ED.EmpCd[Employee Code]
	,	isnull(Emp.Fname,'')[Emp Name ]
	,	ED.SrNo
	,	ED.DocTyp [DocType]
	,	ED.DocNo[DocNo]
	,	ED.IssueDt[IssueDt]
	...
	,	CONVERT(varchar(10), ED.ExpDt,103)[FormatedExpDt]
	,	ED.IssuePlace[IssuePlace]
	,	Cod.SDes[Doc. Type Des]
	,	CONVERT(Varchar(20),ED.TrnDt,103)[FormatedTransDate]
	,	ED.TrnDt[Trans Date ]
	,	case when ED.TrnTyp='I' then 'Issue' else 'Receive' end [TrnTypDes]
	,	ED.TrnTyp
	,	Cod1.SDes[Document Status]
	,	ED.Narr
	,	ED.DocStat[DocStat]
	,	ED.Stat[Status]
	,	ED.EntryBy[EntryBy]
...
,	@v_EmpUser		Char(1)
,	@v_Usercd		varchar(10)
As		-- Drop Procedure [dbo].[EmpLeaveSalaryTicket_Approval_GetRow_N] '','3','01','HR','U'  ,'HR'
Begin	
	Declare @FinalAuth Char(10)
	Select @FinalAuth=Val from Parameters where Cd='LV_APPR_AUTH' and Appcd='HR' and Cocd=@v_CoCd
	Select
	...
	,	CONVERT(varchar(20),TransDt,103)[FormatedTransDt]
	,	EL.EmpCd[EmpCd]  
	,	Rtrim(isnull(FName,''))+' '+Rtrim(isnull(MName,''))+' '+Rtrim(isnull(LName,'')) [Emp]
	,	(select Imagefile from Employee where cd= EL.EmpCd) [ImagePath]
	,	LvSalary
	,	LvTicket
...
Select      
	...
	,	round(IsNull((select isnull(sum(Amt),0) from EmpTrans where EdCd in('TMF1','TMF2','TMF3','TMF4') and EdTyp='HEDT02' and EmpCd=EmT.Empcd  ),0),2)[TMF]
...
Select      
	...
	,	round(IsNull((select isnull(sum(Amt),0) from EmpTransYtd where EdCd in('TMF1','TMF2','TMF3','TMF4') and EdTyp='HEDT02' and EmpCd=EmT.Empcd  and prd=EmT.Prd),0),2)[TMF]
...
,	@v_FromDt		Datetime=''
,	@v_ToDt			Datetime=''
As		-- Drop Procedure [dbo].[Employee_LeaveHistory]'DTD01110','05/01/2024','07/01/2024'
Begin
	...
	begin
		...
		select
			(select SDes from SysCodes where Cd=EMPED.EdTyp)[Type]
		,	(select SDes from CompanyEarnDed where Cd=EMPED.EdCd and Typ=EdTyp)[Des]
		,	AmtVal[AmtVal]
		from 
		EmpEarnDed as EMPED 
		where EmpCd=@v_EmpCd and (EdCd+EdTyp)<>'001HEDT01' 
...
CREATE OR ALTER   procedure [dbo].[EmpTrans_Delete_N]
	@v_EdCd			Char(5)
,	@v_EdTyp		Char(10)
,	@v_Div			Char(5)
,	@v_DocRef		Varchar(10)
,	@v_Empcd		char(10)=''
As
Begin	-- Drop Procedure [dbo].[EmpTrans_Delete] 'SAL101','Holiday OT','Allowances/Earnings','1',''
/*
	Declare	@v_EmpCd		Char(10)
	Declare	@v_EdCdDes		Varchar(30)
	Declare	@v_EdTypDes		Varchar(30)
	Declare	@v_SrNo			Numeric(5)
	Declare	@v_DocRef		Varchar(10)

	set @v_EmpCd='AJ225'
	set @v_EdCdDes='TELEPHONE'
	set @v_EdTypDes='DEDUCTIONS'
	set @v_SrNo=1
	set @v_DocRef='#VC#'

	set @v_EmpCd='SAL101'
	set @v_EdCdDes='Holiday OT'
	set @v_EdTypDes='Allowances/Earnings'
	set @v_SrNo=1
	set @v_DocRef=''
...
*/
	if @v_Empcd in ('ALL','0')
		Set @v_Empcd=''
	Delete from EmpTransDetail where EdCd=@v_EdCd and EdTyp=@v_EdTyp and HRDiv=@v_Div and TrnInd='M' and (EmpCd=@v_Empcd or @v_Empcd='')
	Delete from EmpTrans where EdCd=@v_EdCd and EdTyp=@v_EdTyp and HRDiv=@v_Div and TrnInd='M'
				and (Rtrim(DocRef)=Rtrim(@v_DocRef) or Rtrim(@v_DocRef)='') and( EmpCd=@v_Empcd or @v_Empcd='')
End
 
 Go 

CREATE OR ALTER    Procedure [dbo].[ActivityLogHead_Update_N]
	@v_CoCd			Char(5)
,	@v_ActivityId	bigint=''
,	@v_SessionId	Varchar(40)=''
,	@v_UserCd		Char(10)=''
...
End
 ...
 Go 
CREATE OR ALTER Trigger [dbo].[Trigger_Update_EmpProgression_EmpEarnDed] on [dbo].[EmpProgressionHead] for Update,Insert
As			-- Drop Trigger [Trigger_Update_EmpProgression_EmpEarnDed]
Begin	

		
	Declare @Status			Char(1)
	Declare @StatusOld		Char(1)
	Declare @TransNo		Char(10)
	...
	Declare @EdTyp			Char(10)
	Declare @SrNo			Numeric(5)
	Declare @PercAmt		Char(1)
	Declare @EffDate		Datetime
	Declare @PercVal		Numeric(6,2)
	Declare @EntryBy		Char(5)
	Declare @Curr			Char(5)
	Declare @EntryDt		Datetime
	Declare @EditBy			Char(5)
	

	Select @StatusOld=Status from Deleted
	Select @Status=Status from Inserted

	Select @TransDt=TransDt from Inserted
	Select @TransNo=TransNo from Inserted
	
	Select @EmpCd=EmpCd from Inserted
	Select @EffDate=(select EffDt from empprogressiondetail where transno=@TransNo)
	Select @EntryBy=EntryBy from inserted
	Select @EntryDt=EntryDt from inserted
	Select @EditBy=EditBy from inserted

	if @Status='A' or @StatusOld='A' and @Status='E'
	  Begin
		Declare Incr_Cursor Cursor for
			Select EdTyp,EdCd,EffDt,PercAmt,ApprVal
				From EmpProgressionDetail Where TransNo=@TransNo
		Open InCr_Cursor
		While 1=1
		  Begin
			Fetch next from Incr_Cursor into @EdTyp,@EdCd,@EffDate,@PercAmt,@PercVal
			if @@Fetch_Status=-1 Break
			if @StatusOld='A' and @Status='E'
			  Begin
			--	Delete From EmpEarnDed Where EmpCd=@Empcd and EdCd=@EdCd and EdTyp=@EdTyp and Isnull(TransNo,'')=@TransNo and EffDate=@EffDate
				Update EmpEarnDed Set EntryBy='*DEL*', EditBy='*DEL*' Where EmpCd=@Empcd and EdCd=@EdCd and EdTyp=@EdTyp and 
							Isnull(TransNo,'')=@TransNo and EffDate=@EffDate
			  End
			else
			  Begin
				if (Select Count(1) from EmpEarnDed where EmpCd=@Empcd and EdTyp=@EdTyp and EdCd=@EdCd) > 0
				  Begin
					Select @SrNo=Count(1)+1 from EmpEarnDed where EmpCd=@Empcd and EdTyp=@EdTyp and EdCd=@EdCd
					Select @Curr=Curr from EmpEarnDed where EmpCd=@Empcd and EdTyp=@EdTyp and EdCd=@EdCd
				  End
				else
				  Begin
					Set @SrNo=1		-- Select * from employee
					Select @Curr=CurrCd from Employee where Cd=@Empcd
				  End
				Update EmpEarnDed Set EndDate=Dateadd(D,-1,@EffDate) where EmpCd=@Empcd and EdCd=@EdCd and EdTyp=@EdTyp and SrNo=@SrNo-1
				If @PercAmt='A'
				begin
					print 'insert'
					Insert into EmpEarnDed(EmpCd,EdCd,EdTyp,SrNo,Curr,PercAmt,PercVal,AmtVal,EffDate,EndDate,EntryBy,EntryDt,EditBy,TransNo)
								Values(@EmpCd,@EdCd,@EdTyp,@SrNo,@Curr,@PercAmt,0,@PercVal,@EffDate,'1/1/1900',@EntryBy,@EntryDt,@EditBy,@TransNo)
						
						
					-------------------------------------------------------PROVISION ADJ------------------------------------------------------------------------	
					declare @Purpose		Varchar(50)
					declare @ProvTyp		char(10)
					declare @LvSalAmt		Numeric(18,2)
					Declare @BasicCd		Char(15)
					Declare @CoCd			Char(5)
					Declare	@StrtPrd		Char(6)
					Declare @Prd 			int
					...
					select @Year=Val From Parameters Where Cd='CUR_YEAR' and CoCd=@CoCd
					Exec @EDay=Get_EDay @Prd,@Year
					set @CoCd=(select cocd from employee where cd=@Empcd)
					Select @StrtPrd=Val from Parameters where Cd='HR_ST_PRD' and CoCd=@CoCd
					select @AnnualLv=Val From Parameters Where Cd='CD_PREVILEGE_LV_1' and CoCd='#'
					select @BasicCd='HEDT01'+rtrim(Val) From Parameters Where Cd='BASIC' and CoCd='#'
					set @LvSalAmt=(Select 
								...
								and	EmpCd=Emp.Cd
								and	rtrim(EdTyp)+rtrim(EdCd) <> @BasicCd
								and	rtrim(EdTyp)+rtrim(EdCd) <> rtrim(@EdTyp)+rtrim(@EdCd)
								and	rtrim(EdTyp)+rtrim(EdCd) in (select rtrim(PayTyp)+rtrim(PayCd) From CompanyLeavePay Where LvCd=@AnnualLv and CoCd=@CoCd)
								and	EffDate<=rtrim(@Prd) + '/' + rtrim(@EDay)+'/'+rtrim(@Year) 
								and	(EndDate>=rtrim(@Prd) + '/' + rtrim(@EDay)+'/'+rtrim(@Year)  or EndDate='1/1/1900'))
					set @LvSalAmt=@LvSalAmt+(select Basic from employee where cd=@Empcd)
					set @Purpose='Prov Adj On Employee Progresion'
					set @ProvTyp='LS'
					
					declare @ProvAmt		Numeric(15,4)
					declare @ProvDiff		Numeric(15,4)
					set @ProvAmt=(select 
									Sum(provAmt)
									from EmpProvisions 
									where 
									((Yr*100+Prd>(select top 1 Yr*100+Prd from EmpProvisions where ISNULL(ActAmt,0)>0 and Typ='LS' and Empcd=@Empcd order by Yr*100+Prd desc) 
										and isnull((select top 1 Yr*100+Prd from EmpProvisions where ISNULL(ActAmt,0)>0 and Typ='LS' and Empcd=@Empcd order by Yr*100+Prd desc),'')<>'')
									or (Yr*100+Prd>=@StrtPrd))
									and Yr*100+Prd<datepart(YYYY,@EffDate)*100+datepart(MM,@EffDate)
									and Typ='LS' and Empcd=@Empcd)
					if(@ProvAmt>0)
					begin	
						set @ProvDiff=((@ProvAmt/(@LvSalAmt+(select Val from EmpProgressionDetail where TransNo=@TransNo)))*
								(@LvSalAmt+(select ApprVal from EmpProgressionDetail where TransNo=@TransNo)))-@ProvAmt
								
						if(@ProvDiff>0)	
							exec Update_Empprovisionsadj @EffDate,@TransNo,@TransDt,@Empcd,@ProvTyp,0,@ProvDiff,@Purpose,@Purpose
					-------------------------------------------------------PROVISION ADJ------------------------------------------------------------------------
				...
				begin
					Insert into EmpEarnDed(EmpCd,EdCd,EdTyp,SrNo,Curr,PercAmt,PercVal,AmtVal,EffDate,EndDate,EntryBy,EntryDt,EditBy,TransNo)
								Values(@EmpCd,@EdCd,@EdTyp,@SrNo,@Curr,@PercAmt,@PercVal,0,@EffDate,'1/1/1900',@EntryBy,@EntryDt,@EditBy,@TransNo)
				end
			  End
		  End
		Close InCr_Cursor
	...
	if @StatusOld='A' and @Status='E'
	  Begin
		Delete From EmpEarnDed Where EditBy='*DEL*' and EntryBy='*DEL*' and EmpCd=@Empcd and Isnull(TransNo,'')=@TransNo
	  End
End
 
 Go 
CREATE OR ALTER Procedure [dbo].[Branch_Count]
@v_CoCd char(5)
...
End 
 Go 
CREATE OR ALTER PROCEDURE [dbo].[Get_UserBranches]
@v_CoCd char(5),
@v_Abbr char(5)
as
begin
...
Begin	
	...
	,	MessageBody
	,	(select STRING_AGG(trim(EmpCd), ',') from NotificationDetail_N where ProcessId = NM.ProcessId and DocTyp=NM.DocTyp)Attendees
	FROM NotificationMaster_N  as NM
	WHERE  @v_typ='0'
...
End 
 ...
 Go 
CREATE OR ALTER  
 Procedure [dbo].[EmployeeAuto_GetRow_N]
as
--drop proc EmployeeAuto_GetRow_N
Begin
	declare @v_LEN_EMPCD char(1)
	declare @v_ST_EMPCD  char(10)
	declare @v_LEN_ST_EMPCD  char(1)
	declare @v_Empcd  char(10)
	declare @v_Empcd1 char(10)
	declare @v_Empcd2 char(10)
	declare @v_Empcd3 char(10)
	declare @v_Empcd4 char(10)
	declare @v_Add		int=1
	select  @v_LEN_EMPCD= val from parameters where cd='LEN_EMPCD'
	select  @v_ST_EMPCD= ltrim(rtrim(val)) from parameters where cd='ST_EMPCD'
	select  @v_LEN_ST_EMPCD= len(@v_ST_EMPCD)
	select @v_Empcd=isnull(max(cd),0) from employee--maximum employee code
	if(@v_Empcd='0')
	begin
		set @v_Empcd=@v_Empcd+1
		set @v_Empcd= REPLICATE('0', (@v_LEN_EMPCD-LEN(@v_Empcd)))+@v_Empcd
		select ltrim(rtrim(@v_ST_EMPCD))+@v_Empcd
	end
	else
	begin
		select @v_Empcd1=SUBSTRING(@v_empcd,@v_LEN_ST_EMPCD+1,len(@v_Empcd))--employee code without initial string
		set  @v_Empcd2=@v_Empcd1+1 --next employee code 
		select @v_Empcd3= REPLICATE('0', (@v_LEN_EMPCD-LEN(@v_Empcd2)))+@v_Empcd2--appending zeros to the left of new employeecode to match with the series
		select @v_Empcd4=SUBSTRING(@v_empcd,1,len(@v_Empcd)-@v_LEN_EMPCD)+@v_Empcd3--Generating new employeecode by adding the initial charecter
		select @v_Empcd4
	end
