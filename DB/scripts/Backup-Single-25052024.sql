...
AS
--Drop Procedure [dbo].[EmplLoanAndLeaveApproval_N]'01','488','E','523'

select COUNT(TransNo) as LeaveApplied from EmpLeave
inner join Employee as emp on emp.Cd=EmpLeave.EmpCd
where GETDATE() between FromDt and ToDt and LvStatus='N' and emp.CoCd=@v_CoCd 
and emp.div in(select div from UserBranch where usercd=@v_Usercd)--and emp.Status='HSTATPM   '
