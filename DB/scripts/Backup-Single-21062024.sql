...
As		-- Drop Procedure [dbo].[GetRepo_EmpShortList_N] '100','All ','All ','All ','All ','All ','All','0','All','ALL','Y','All','ALL','001'
Begin
	Select
	...
	,	Emp.EmpTyp
	,	Emp.Personal_No[PersonalNumber]
	from 
		Employee Emp
...
BEGIN
	...
	delete from AdImages where UserCd = @v_UserCd and (Cd=@v_Cd or @v_Cd=0)
END
 
 Go 
CREATE OR ALTER      Procedure [dbo].[AdImages_Update]
	@v_Cd				int
,	@v_UserCd		varchar(10)
...
Begin
	...
	IF (SELECT COUNT(*) FROM AdImages WHERE UserCd = @v_UserCd and Cd = @v_Cd) = 0
	  Begin
...
-- =============================================
CREATE OR ALTER PROCEDURE [dbo].[AdImages_GetRow]
	@v_UserCd varchar(10)='',
	@v_Cd int = 0
AS
BEGIN
    ...
    -- Insert statements for procedure here
	select * from AdImages where UserCd = @v_UserCd and (Cd = @v_Cd or @v_Cd=0)
END
...
BEGIN
    ...
    -- Insert statements for procedure here
	Select *
	,	(select name from Counters where Cd = t.CounterCd)[CounterName]
...
select count(cd)[Working] from Employee where  (Status  in ('HSTATPM','HSTATTG   ','HSTATTP   ')) 
and cd not in(select distinct EmpCd from empleave where LvStatus='F' and ISNULL(JoinDt,'01/01/1900')='01/01/1900') 
and active='Y'--1225
...
,	@v_RYear		Char(4)
,	@v_EmpCd		Char(10)
As		-- Drop Procedure [dbo].[GetRepo_PaySlip]'100','All','03','2020','MHM/9500  '
Begin
/*
...
CREATE OR ALTER     Procedure [dbo].[ActivityLogHead_Getrow]
	...
	@v_EndDate		datetime=''
As		-- Drop Procedure [dbo].[ActivityLogHead_Getrow] '1',null,null
Begin
...
CREATE OR ALTER     Procedure [dbo].[ActivityLogDetail_Getrow]
	@v_ActivityId		bigint=''
As		-- Drop Procedure [dbo].[ActivityLogDetail_Getrow] '5'
Begin
...
,		@query  AS NVARCHAR(MAX)
	If Cast(@v_RYear as int) *100 +Cast(@v_RPrd as int) >= @Year *100 +@Month
		BEGIN
			...
			,	(Select replace(des,' ','') from CompanyEarnDed where Cd=EmT.EdCd and Typ=EmT.EdTyp)[Sdes]
			,	isnull(Amt,0)[Amt]
			...
			,	@Days[WorkingDays]
			,	(Select replace(des,' ','') from CompanyEarnDed, EmpTrans EmT where Cd=EmT.EdCd and Typ=EmT.EdTyp and  EmpCd=Emp.Cd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=Emp.Div))[Sdes]
			,	0[Amt]
			...
			Set @query=N'Select cd,Empname,Prd,WorkingDays,ElSal,LOP,Yr,Desg,DOj,Dept,secttion,Paymode,Sponsor,Branch,'+@pivotcols+'  from #temp_Sal Pivot (sum(Amt) for Sdes in('+@pivotcols+')) as Pivot1'
	...
	Else
		BEGIN
			...
			,	round(IsNull((Select sum(Amt) from EmpTransYtd where EdCd='004'  and EdTyp='HEDT01' and prd=@Prd and EmpCd=EmT.Empcd and (@v_Branch='All' or @v_Branch<>'' and HRDiv=EmT.HRDiv)),0),2)[Other Allowance]
		...
		--	and Emp.Active='Y'
			...
			,	round(IsNull((Select sum(Amt) from EmpTransYtd where EdCd='004'  and EdTyp='HEDT01' and prd=@Prd and EmpCd=Emp.Cd and (@v_Branch='All' or @v_Branch<>'' and HRDiv=emp.Div)),0),2)[Other Allowance]
			,	(Emp.Basic/@Days)*
				+(IsNull((Select top 1 isnull(AmtVal,0) from EmpEarnDed where EdCd='004' and EdTyp='HEDT01' and	EffDate <= rtrim(@Month) + '/' + rtrim(@EDay)+'/'+rtrim(@Year)
			...
			,	@Days[WorkingDays]
			,	(Select replace(des,' ','') from CompanyEarnDed, EmpTransYtd EmT where Cd=EmT.EdCd and prd=@Prd and Typ=EmT.EdTyp and  EmpCd=Emp.Cd and (@v_Branch='All' or @v_Branch<>'All' and HRDiv=Emp.Div))[Sdes]
			,	0[Amt]
			,	Cast(@v_RPrd as CHAR(2))[Prd]
			,	Cast(@v_RYear as CHAR(4))[Yr]
			--,	(Select CoName from Company where Cd=@v_CoCd)[CompanyName]
			...
			Set @query=N'Select cd,Empname,Prd,WorkingDays,ElSal,LOP,Yr,CompanyLogo,Desg,DOj,Dept,secttion,Paymode,Sponsor,Branch,'+@pivotcols+'  from #temp_Sal1 Pivot (sum(Amt) for Sdes in('+@pivotcols+')) as Pivot1'
			exec sp_executesql @query
...
@v_EmpCd char(10)
AS --Drop Procedure [dbo].[EmplLoanAndLeaveHistory]'655/ALH   '
...
Begin
	...
	Delete from EmpDocImages where EmpCd = @v_EmpCd and DocTyp=@v_DocTyp and (SlNo = @v_srNo or @v_srNo=0)
